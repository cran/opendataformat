...
The output of the `read_odf()` function is an R-data.frame object, that has the additional class `odf`. It has additional metadata stored in the attributes of the dataframe and the variables/columns. These include the languages of the metadata, labels, descriptions, urls, variable types, and value labels.
